...
xdata float ical[12] = {2.623115577889436,422.5721784776902,846.0043196544276,1260.6598984771572,1681.5789473684208,2081.578947368421,2497.435897435897,2893.6170212765956,3320.0,3772.7272727272734,4272.727272727276,4272.727272727276};
//-----------------------------------------------------------------------------
// Global Variables
//-----------------------------------------------------------------------------
...
// Encoder signals handling
    ...
    if (encoder_counter == ENCODER_SCAN_RATE)
      ...
      dex = amp_counter.c/50;
      temp = ical[dex];
      temp += (ical[dex+1]-ical[dex])*((amp_counter.c)%50)/50.0;
      dacA = temp;
//      dacA = amp_counter.c;
